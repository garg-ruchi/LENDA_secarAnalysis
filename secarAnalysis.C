...
//=====================================================================//
  ...
  // LScin histograms
  TList *LScin = new TList();
  TH1D *h_m_LScin = new TH1D("h_m_LScin","LScin multiplicity",20,0,20);
  LScin->Add(h_m_LScin);
  TH1D *h_e_LScin = new TH1D("h_e_LScin","energy LScin",2000,0,100);
  LScin->Add(h_e_LScin);
  TH1D *h_t_LScin = new TH1D("h_t_LScin","time LScin",2000,0,100);
  LScin->Add(h_t_LScin);
  ...
  TH2D *h_ct_MCP_Si = new TH2D("h_ct_MCP_Si","Si strip F vs time difference between MCPs and Si",250,0,1000,32,0,32);
  ...
  // LScin - Si conincidence histograms
  ...
  TH2D *h_ec_LScin_Si = new TH2D("h_ec_LScin_Si","Si strip vs Si energy",4000,0,200000,32,0,32);
  LScin_Si->Add(h_ec_LScin_Si);
...
//=====================================================================//
  ...
  double blob1a_Si[2], blob1a_dE[2], blob1b_Si[2], blob1b_dE[2], blob2_Si[2], blob2_dE[2];
...
//-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-o-//
  ...
  for (Long64_t jentry=0; jentry<nentries;jentry++) {
    ...
    LScinHit = sortLScintHits(mult_LScin, energy_LScin, chan_LScin, time_LScin);
...
//==========coincidence histograms==========//
  ...
  //==========Si LScin coincidence==========//
        if(LScinHit.time.size()>0){
          for(int j=0; j<LScinHit.time.size(); j++){
            ...
            if((SiHit.time_F[i]-LScinHit.time[j])>1200 && (SiHit.time_F[i]-LScinHit.time[j])<1700) h_ec_LScin_Si->Fill(SiHit.energyCal_F[i], SiHit.strip_F[i]);
          }
        }
  //==========Si RFQ coincidence==========//
  ...
  LScin->Write("LScin",TObject::kSingleKey);
  ...
  LScin_Si->Write("LScin_Si",TObject::kSingleKey);
  RFQ_Si->Write("RFQ_Si",TObject::kSingleKey);
  LENDA_IC_Si->Write("LENDA_IC_Si",TObject::kSingleKey);
